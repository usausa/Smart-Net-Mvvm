...
{
    private static readonly Assembly SmartMvvmAssembly = typeof(ObservablePropertyAttribute).Assembly;
    ...
    public static IReadOnlyList<Diagnostic> GetDiagnostics(string source) =>
        RunGenerator(source).Diagnostics.Where(d => d.Id.StartsWith("SMV", StringComparison.Ordinal)).ToList();
    ...
    private static (IEnumerable<Diagnostic> Diagnostics, string GeneratedSource) RunGenerator(string source)
    {
        ...
        var references = new[]
        {
            ...
            MetadataReference.CreateFromFile(SmartMvvmAssembly.Location)
